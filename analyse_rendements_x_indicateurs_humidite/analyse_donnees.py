...
def tracer_cartes_moyennes_annuelles():
    ...
    gdf_departements.plot(ax=axes[1, 1], column='humidite_mm_MOYEN', cmap='viridis_r', edgecolor='black', linewidth=0.5, legend=True)
    axes[1, 1].set_title('humidite_mm')
...
def tracer_cartes_par_annees():
    ...
    for a, annee in enumerate(ANNEES):
        ...
        gdf.plot(ax=axes[a, 1], column='SWI_UNIF_MENS3_ECART_RELATIF', cmap='RdBu', edgecolor='black', linewidth=0.5, legend=True, vmin=-30,
                 vmax=30)
        axes[a, 1].set_title(f'{annee} - SWI_UNIF_MENS3_ECART_RELATIF {MOIS}')
...
def tracer_correlations_moyennes():
    ...
    scatter_matrix(df_departements.loc[:, ['CULT_REND_MOYEN', 'SWI_UNIF_MENS3_MOYEN', 'reserve_mm', 'humidite_mm_MOYEN']], alpha=0.8, figsize=(6,
                                                                                                                                               6), diagonal='kde')
    plt.tight_layout()
...
def tracer_correlations():
    ...
    lignes = ['SWI_UNIF_MENS3', 'humidite_mm', 'SWI_UNIF_MENS3_ECART_ABSOLU', 'SWI_UNIF_MENS3_ECART_RELATIF']
    colonnes = ['CULT_REND', 'CULT_REND_ECART_ABSOLU', 'CULT_REND_ECART_RELATIF']
    fig, axes = plt.subplots(nrows=len(lignes), ncols=len(colonnes), figsize=(20, 12))
...
def tracer_correlations_par_departements():
    ...
    lignes = ['SWI_UNIF_MENS3', 'humidite_mm']
    colonnes = ['CULT_REND', 'CULT_REND_ECART_ABSOLU', 'CULT_REND_ECART_RELATIF']
    fig, axes = plt.subplots(nrows=len(lignes), ncols=len(colonnes), figsize=(20, 12))
